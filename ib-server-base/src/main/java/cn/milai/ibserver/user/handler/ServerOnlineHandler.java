...
import cn.milai.ibserver.user.conf.ChannelAttr;
import cn.milai.ibserver.user.conf.RedisKey;
...
public class ServerOnlineHandler implements OnlineHandler {
	...
	public Long getUserId(ChannelHandlerContext ctx) {
		return Channels.longAttr(ctx, ChannelAttr.ATTR_USER_ID);
	}
	...
	private void online(long userId, ChannelHandlerContext ctx) {
		...
		Channels.setAttr(ctx, ChannelAttr.ATTR_USER_ID, userId);
	}
	...
	private Long offline(ChannelHandlerContext ctx) {
		Long userId = Channels.longAttr(ctx, ChannelAttr.ATTR_USER_ID);
		if (userId != null) {
